...
def check_fields(data_path):
	...
	for data_item in data_items:
		if 'video_id' not in data_item:
			print(f"submitted prediction json file doesn't have required 'valid_id' field for {data_item}!")
			sys.exit(0)
		if 'label' not in data_item:
			print(f"submitted prediction json file doesn't have required 'label' field for {data_item}!")
			sys.exit(0)
		
		if data_item['video_id']=='' or data_item['video_id']==None:
			print("'valid_id' field can not be empty")
			sys.exit(0)

		if data_item['label']=='' or data_item['label']==None:
			print("'label' field can not be empty")
			sys.exit(0)
